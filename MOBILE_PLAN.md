...
## Продолжение плана: релизный трек (GitHub → телефон)

### Что добавлено в репозиторий
- Runbook релиза: `docs/MOBILE_RELEASE_RUNBOOK.md`
- CI workflow preview сборок: `.github/workflows/mobile-eas-preview.yml`
- CI workflow production сборок: `.github/workflows/mobile-eas-production.yml`
- CI workflow submit в сторах: `.github/workflows/mobile-eas-submit.yml`
- Базовый `apps/mobile/eas.json` с профилями `preview` и `production`

### Ближайшие шаги (в порядке выполнения)
1. Инициализировать Expo приложение в `apps/mobile` и закоммитить базовую структуру.
2. Подключить Supabase Auth и минимальный navigation skeleton (auth + dashboard).
3. Заполнить GitHub Secrets для EAS и store submit.
4. Проверить preview pipeline на ветке `develop`.
5. Выпустить первый RC-тег `v0.1.0-rc.1` и проверить production build pipeline.

### Критерий готовности к релизу
- Android APK устанавливается на физический телефон по ссылке из EAS.
- iOS билд доступен через TestFlight для тестеров.
- Сборки воспроизводимы через GitHub Actions без локальной ручной сборки.
